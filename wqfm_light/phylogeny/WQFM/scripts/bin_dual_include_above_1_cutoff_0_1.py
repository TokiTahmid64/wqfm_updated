...
def find_in_bins(list_ratios, thresh = 0.9):
    ...
    if (total_counts_AFTER_thresh_below_1 + total_counts_after_or_equal_1) == 0:  # >=1  has no 4-tax-seq pree
        weighted_avg_bin_ratio_after_thresh = -1
    else:
        # SUM(mid-point-bin-counts upto (THRESH, 1)) + SUM(1* [>=1-counts] after >= 1)
        weighted_avg_bin_ratio_after_thresh = (cumulative_weighted_midpoint_after_thresh + 1*total_counts_after_or_equal_1)/(total_counts_AFTER_thresh_below_1 + total_counts_after_or_equal_1)
        # weighted_avg_bin_ratio_after_thresh = cumulative_weighted_midpoint_after_thresh/total_counts_AFTER_thresh_below_1

    return weighted_avg_bin_ratio_before_thresh, weighted_avg_bin_ratio_after_thresh, total_counts_before_thresh, total_counts_AFTER_thresh_below_1, total_counts_after_or_equal_1, total_counts_all, dictionary_bins
...
if num_total_four_tax_seq == 0:
    # print(f"{weighted_avg_bin_ratio_before_thresh} {weighted_avg_bin_ratio_after_thresh} {total_counts_before_thresh/total_counts_all} {total_counts_AFTER_thresh_below_1/total_counts_all}")
    pass
else:
    weighted_avg_bin_ratio_before_thresh, weighted_avg_bin_ratio_after_thresh, total_counts_before_thresh, total_counts_AFTER_thresh_below_1, total_counts_after_or_equal_1, total_counts_all, dictionary_bins = find_in_bins(list_ratios, float(sys.argv[2]))


CUT_OFF_LIMIT = 0.1 # consider 10% as cut-off limit [reasonable enough.] (>=10%, bin left (0.5, THRESH), else bin right (THRESH, 1))
...
# else:
#     print(f"{num_four_tax_seq_with_3_qrts/num_total_four_tax_seq} {weighted_avg_bin_ratio_before_thresh} {weighted_avg_bin_ratio_after_thresh} {total_counts_before_thresh/total_counts_all} {total_counts_AFTER_thresh_below_1/total_counts_all} {BETA}")
