...
def find_in_bins(list_ratios):
    ...
    idx_bin = 0
    total_weights_to_divide_without_1 = 0
    cumulative_weighted_mid_ratio_without_1 = 0
    for _bin in bins:
        (lower_lim, upper_lim) = _bin # unwrap
        if lower_lim != 1: # ALL ARE WITHOUT 1
            mid_point = 0.5*(lower_lim + upper_lim)
            weighted_mid_ratio = mid_point*dictionary_bins[_bin]
            ####### compute cumulative things ########
            total_weights_to_divide_without_1 += dictionary_bins[_bin]
            cumulative_weighted_mid_ratio_without_1 += weighted_mid_ratio

    weighted_avg_bin_ratio = cumulative_weighted_mid_ratio_without_1/total_weights_to_divide_without_1


    return weighted_avg_bin_ratio, normal_range_counts, last_range_counts, total_counts_all, dictionary_bins
...
weighted_avg_bin_ratio, normal_range_counts, last_range_counts, total_counts_all, dictionary_bins = find_in_bins(list_ratios)

if len(sys.argv) == 2:
    # print(f"{weighted_avg_bin_ratio} {normal_range_counts/total_counts_all} {last_range_counts/total_counts_all}")
    print(f"{weighted_avg_bin_ratio}")
